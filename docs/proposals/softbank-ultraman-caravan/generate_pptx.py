...
add_table(s, Inches(0.7), Inches(1.75), Inches(11.9),
          ...
          [
              ...
              ["短期IP使用料(A案:1,200万円)",
               "年間契約(B案)の月額水準・約670万円がベースレート。標準では3〜4か月で約2,400万円のところ、非独占・2〜3施設・限定媒体の条件かつB案(年間契約)への移行を前提とした導入特別価格(約50%)として設定"],
              ["ヒーロー登場・イベント運営費",
...
add_table(s, Inches(0.7), Inches(1.75), Inches(6.0),
          ["見積項目", "金額"],
          [
              ["ウルトラマンIP使用料・監修費(導入特別価格)", "1,200万円"],
              ["企画設計・PM", "250万円"],
              ["キービジュアル・コピー制作", "300万円"],
              ["LP/応募フォーム制作", "350万円"],
              ["館内ラリーツール制作", "250万円"],
...
textbox(s, Inches(0.7), Inches(6.35), Inches(11.9), Inches(0.5), [
    ("※IP使用料1,200万円は、標準価格約2,400万円(B案の月額水準×3〜4か月)に対し、B案(年間契約)への移行を前提とした導入特別価格です。ヒーロー登場・運営費は2〜3施設×週末開催(計8〜12回想定)で積算しています。", 11, SUB, False),
])
...
# =============================================================================
s = add_slide()
header(s, "実施体制", "貴社と円谷プロダクション(企画・稼働・監修)+制作パートナーの連携体制で推進します")
add_table(s, Inches(0.7), Inches(1.7), Inches(11.9),
          ["役割", "担当"],
          [
              ["全体統括", "SoftBank コンシューマ事業側"],
              ["企画・プロデュース", "円谷プロダクション(企画開発チーム)"],
              [("ヒーロー稼働・ショー運営", True, INK), ("円谷プロダクション(自社アクター・造形体制による安定供給)", True, INK)],
              ["IP監修・ライセンス管理", "円谷プロダクション"],
              ["制作(KV・LP・ラリーツール)", "円谷プロダクション監修のもと、制作パートナー各社"],
              ["デジタル隊員証・計測導線", "円谷プロダクション デザインチーム(内製想定)"],
          ...
          col_widths=[1.1, 2], row_h=0.42, font_size=12)
...
# Appendix 中扉
# =============================================================================
divider("APPENDIX", "Appendix",
        "KPI設定の積算根拠・前提条件・投資対効果の考え方")

# =============================================================================
# A1. KPI積算根拠①:1開催日あたりへの分解
# =============================================================================
s = add_slide()
header(s, "Appendix①:KPI積算根拠 — 年間KPIの「1開催日あたり」分解(B案)",
       "年間80〜160開催日(10〜20施設×週末1〜2回)で分解し、モール型キャラクターイベントの運用実勢と突合しています")
add_table(s, Inches(0.7), Inches(1.6), Inches(11.9),
          ["KPI(B案・年間)", "1開催日あたり", "実勢・設計上の裏付け"],
          [
              ["総イベント来場 3万〜8万人", "約190〜1,000人/日",
               "モール型ショーの標準運用(30分ステージ+撮影会を1日2回、1回200〜400人観覧)で1日400〜800人が実勢"],
              ["撮影会参加 3,000〜8,000組", "約19〜100組/日",
               "撮影会は1回あたり20〜90組の入場制限運用が一般的。1日1〜2回で最大180組"],
              ["館内ラリー参加 1万〜3万人", "約63〜375人/日",
               "来場者の33〜38%と設定(ファミリー主体の参加型企画の設計目標値)"],
              ["デジタル隊員証 8,000〜2万人", "—",
               "ラリー動線に発行ミッションを組込(ラリー参加者の67〜80%)"],
              ["ショップ送客 3,000〜8,000人", "約19〜100人/日",
               "「ショップ基地=最終ミッション」設計。ラリー完走率約3割と設定"],
              ["相談 1,000〜3,000件", "約6〜38件/日",
               "店舗の相談キャパシティ(3〜4卓×30分×8時間=約50〜60件/日)の範囲内で設定"],
              ["見積・来店予約 500〜1,500件", "—",
               "相談からの見積化率5割と設定(店頭相談の一般水準3〜5割の上限側)"],
          ],
          col_widths=[1.15, 0.75, 2.6], row_h=0.55, font_size=11)
textbox(s, Inches(0.7), Inches(6.15), Inches(11.9), Inches(0.6), [
    ("※公開されているモール型キャラクターイベントの運用実勢、および店舗オペレーションの設計値に基づく積算です。転換率はFY27期首の検証フェーズで実測し、年間KPIを精緻化します。", 10.5, SUB, False),
])

# =============================================================================
# A2. KPI積算根拠②:KPIツリーと下限・上限の定義
# =============================================================================
s = add_slide()
header(s, "Appendix②:KPIツリーと下限・上限の定義",
       "各KPIは転換率で連結したツリー構造です。下限は必達水準、上限はフル運用時のストレッチ水準として設定しています")
flow_down(s, Inches(0.85), Inches(1.7), Inches(6.2), [
    "総イベント来場(100%)",
    "館内ラリー参加(来場の33〜38%)",
    "デジタル隊員証発行(参加の67〜80%)",
    "ショップ送客(ラリー完走 約3割)",
    "相談(送客の33〜37%)",
    "見積・来店予約(相談の約5割)",
    "KGI:新規契約(モバイル・BB・でんき)",
], box_h=0.5, gap=0.17, size=12, first_fill=NAVY, last_fill=RED)
b = rect(s, Inches(7.5), Inches(1.7), Inches(5.3), Inches(2.4), fill=PALE, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
b.adjustments[0] = 0.05
textbox(s, Inches(7.75), Inches(1.9), Inches(4.8), Inches(2.0), [
    ("下限・上限の定義", 13, RED, True),
    ("・下限:10施設×週1回(年80開催日)の必達水準", 12, INK, False),
    ("・上限:20施設×週2回(年160開催日)のストレッチ水準", 12, INK, False),
    ("・幅は「計画のブレ」ではなく、施設数・開催頻度の運用レンジに対応", 12, INK, False),
], space_after=8)
b2 = rect(s, Inches(7.5), Inches(4.3), Inches(5.3), Inches(2.15), fill=PALE_RED, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
b2.adjustments[0] = 0.05
textbox(s, Inches(7.75), Inches(4.5), Inches(4.8), Inches(1.8), [
    ("検証による精緻化", 13, RED, True),
    ("・FY27期首(A案フェーズ)で全転換率を実測", 12, INK, False),
    ("・デジタル隊員証のQR発行・店舗チェックインが計測基盤", 12, INK, False),
    ("・実測値で年間KPIを更新し、月次レポートで進捗を開示", 12, INK, False),
], space_after=8)
...
# =============================================================================
s = add_slide()
header(s, "Appendix③:ご参考 — 投資対効果の考え方(世帯LTVベース)",
       "本企画の狙いは単回線の獲得ではなく、家族契約+BB+でんきの「世帯バンドル」獲得です")
add_table(s, Inches(0.7), Inches(1.75), Inches(11.9),
          ["ステップ", "参考試算"],
          [
              ["① 見積・来店予約(B案・年間)", "500〜1,500件"],
              ["② 成約率(店頭+後日オンライン転換)", "3〜5割 → 年間150〜750世帯の新規獲得"],
              ["③ 世帯あたり月額(モバイル2回線+光/Air+でんきのバンドル想定)", "約15,000円/月"],
              [("④ 世帯LTV(36か月継続想定)", True, INK), ("約54万円/世帯", True, RED)],
              [("⑤ 年間売上創出規模(②×④)", True, INK), ("約8,000万円〜4億円", True, RED)],
          ],
          col_widths=[1.8, 1.4], row_h=0.52, font_size=12.5)
box = rect(s, Inches(0.7), Inches(4.85), Inches(11.9), Inches(1.1), fill=PALE_RED)
textbox(s, Inches(1.0), Inches(5.0), Inches(11.3), Inches(0.85), [
    ("さらに、場所代の削減効果(本編「実質負担の考え方」参照)と、来店意向のない層との接点創出・既存顧客の定着効果を加味すると、", 13, INK, False),
    ("本企画の投資対効果は獲得単価の単純比較を大きく上回ります。", 14, RED, True),
], space_after=4)
textbox(s, Inches(0.7), Inches(6.15), Inches(11.9), Inches(0.5), [
    ("※世帯単価・成約率・継続期間は一般的な水準に基づく参考試算であり、成果をお約束するものではありません。長期利用ユーザーの獲得・定着という貴社コンシューマ事業方針に沿った評価軸としてご提示するものです。", 10.5, SUB, False),
])
