...
textbox(s, Inches(0.85), Inches(1.86), Inches(11.7), Inches(0.9), [
    ...
    [("SoftBankショップ、BB、でんき、モバイル相談へ送客する。", 15, INK, True),
     ("本命はFY27年度の年間活用を前提とした1.2億円プラン(B案)。", 15, RED, True)],
], space_after=4)
bullets(s, Inches(0.85), Inches(3.15), Inches(11.8), Inches(3.6), [
    ("背景:現行ポケモン契約(FY26:7,000万円)はイベント・BB・でんきで活用可能。FY27以降も契約は継続予定だが、着ぐるみ利用は不可となる前提。", {}),
    ("課題:FY27以降、商業施設イベントにおける「会いに行く理由」=リアル集客機能に空白が生まれる。", {}),
    ("解決:ポケモンの置き換えではなく、リアルイベント集客機能をウルトラマンで補完する。2026年7月10日にシリーズ60周年を迎える追い風もある。", {}),
    ("仕組み:ヒーロー撮影会・館内ミッションラリー・デジタル隊員証で商業施設に呼び込み、SoftBankショップの相談(スマホ・BB・でんき)へ自然に送客。", {}),
    ("提案:A案3,500万円(短期検証)/B案1億2,000万円(FY27年間・本命)/C案2億5,000万円(60周年大型展開)。", {"bold": True}),
], size=14, gap=12)

# =============================================================================
# 3. 提案ポジション(高島副社長管掌領域との接続)
# =============================================================================
s = add_slide()
header(s, "ご提案のポジション:コンシューマ事業のリアル接点拡張施策",
       "「IPコラボ」ではなく、コンシューマ事業統括の管掌領域に直結する成長施策としてご提案します")
bullets(s, Inches(0.85), Inches(1.75), Inches(11.8), Inches(1.7), [
    "高島謙一氏は2026年4月1日付で、副社長執行役員 兼 COO コンシューマ事業統括に就任。",
    "ソフトバンクのコンシューマ事業は、付加価値サービスを備えた料金プランの浸透、グループ経済圏の活用、長期利用が期待できるユーザー層の獲得・定着を通じ、モバイルサービス売上とセグメント利益の継続成長を目指す方針。",
    "SoftBank/ワイモバイル/LINEMOのマルチブランド展開も競争優位として提示されている。",
], size=13.5, gap=10)
...
# =============================================================================
s = add_slide()
header(s, "現行ポケモン契約の整理", "FY27以降も契約は継続予定。ただし着ぐるみ利用が不可となり、リアル集客機能に課題が残ります")
add_table(s, Inches(0.7), Inches(1.75), Inches(8.1),
          ["項目", "現状"],
          [
              ["契約期間", "4月〜3月の年度単位"],
              ["FY26契約費用", ("7,000万円", True, RED)],
              ["利用可能領域", "イベント、BB、でんき"],
              ["ノベルティ", "作成数制限なし(数量に応じた許諾対価が別途必要)"],
              ["他コンテンツ制限", "なし(他IPとの併用が可能)"],
              ["着ぐるみ利用", ("FY26 3月まで", True, INK)],
              ["FY27以降", ("契約継続予定。ただし着ぐるみ利用不可", True, RED)],
              ["他IP検討状況", "ちいかわ、すみっコぐらし、サンリオにアプローチ中"],
          ],
...
textbox(s, Inches(9.3), Inches(2.0), Inches(3.2), Inches(3.4), [
    ("ここからの論点", 14, RED, True),
    ("① ポケモンはBB・でんき・既存コンテンツで継続すべき", 12.5, INK, False),
...
textbox(s, Inches(0.85), Inches(6.05), Inches(11.8), Inches(0.7), [
    [("答え:", 15, RED, True),
     ("ポケモンを置き換えるのではなく、FY27以降担いにくくなる“リアルイベント集客機能”をウルトラマンで補完する。", 15, INK, True)],
])
...
add_table(s, Inches(0.7), Inches(1.85), Inches(11.9),
          ["IP", "役割"],
          [
              ["ポケモン", "BB・でんき・既存コンテンツで継続活用(FY27以降も契約継続)"],
              ["ちいかわ/すみっコぐらし/サンリオ", "かわいい系、SNS拡散、女性・低年齢層向け"],
...
# =============================================================================
s = add_slide()
header(s, "未来防衛隊ミッションラリー", "館内回遊と相談接点を同時に生む、本企画の中核コンテンツ")
...
textbox(s, Inches(0.85), Inches(5.55), Inches(11.8), Inches(1.0), [
    [("ノベルティ設計:", 13.5, RED, True),
     ("ポケモン契約と同様、作成数に上限を設けず、数量に応じた許諾対価を別途支払う設計が自然。", 13.5, INK, False)],
])
...
bullets(s, Inches(0.85), Inches(1.75), Inches(11.8), Inches(0.9), [
    "PayPayとLINEヤフーは、LINE(国内月間利用者数1億)とPayPay(登録ユーザー7,400万人)のアカウント連携を2026年夏より開始と発表。",
], size=13.5)
...
# =============================================================================
s = add_slide()
header(s, "3プラン比較", "本命はB案:FY27標準プラン(1億2,000万円)です")
add_table(s, Inches(0.7), Inches(1.85), Inches(11.9),
          ["プラン", "金額(税別)", "位置づけ", "推奨度"],
          [
              ["A案:短期検証プラン", "3,500万円", "商業施設イベントの小規模検証(FY26下期パイロット)", "○"],
              [("B案:FY27標準プラン", True, RED), ("1億2,000万円", True, RED), ("年間展開の本命。着ぐるみ不可後のリアル集客を補完", True, INK), ("◎", True, RED)],
...
textbox(s, Inches(1.0), Inches(5.08), Inches(11.3), Inches(1.1), [
    ("推奨の進め方", 14, RED, True),
    ("A案パイロット(FY26下期)で反応を検証 → B案でFY27年間展開 → 成果に応じてC案へ拡張。", 15, INK, True),
], space_after=6)
...
# =============================================================================
s = add_slide()
header(s, "A案:短期検証プラン|3,500万円(税別)", "FY27本格展開前に、ウルトラマンを核にした商業施設送客の反応を検証(3〜4か月)")
...
textbox(s, Inches(7.1), Inches(5.05), Inches(5.6), Inches(1.6), [
    ("位置づけ", 13, RED, True),
    ("稟議上の入口として有効。ただし着ぐるみ不可後の空白を本格的に補完するには規模が小さい。", 12.5, INK, False),
], space_after=6)
...
# =============================================================================
s = add_slide()
header(s, "B案:FY27標準プラン|1億2,000万円(税別)【本命】",
       "FY27年度(2027年4月〜2028年3月)/商業施設10〜20施設/ヒーロー年間稼働枠/BB・でんき本格導線化/他IP併用可")
add_table(s, Inches(0.7), Inches(1.75), Inches(6.0),
          ["見積項目", "金額"],
          [
              ["年間IP使用料・監修費", "5,000万円"],
...
# =============================================================================
s = add_slide()
header(s, "B案が本命である理由", "ポケモン契約7,000万円との単純比較ではなく、「年間IP契約+リアルイベント実行費+店舗送客導線」の総額として評価すべきです")
textbox(s, Inches(0.85), Inches(1.75), Inches(11.8), Inches(0.8), [
    ("ポケモンFY26契約費用7,000万円は年度IP契約の社内ベンチマークとして有効。ただし、B案1億2,000万円はIP使用料だけの金額ではありません。", 14, INK, False),
], space_after=4)
items = [
...
header(s, "C案:60周年大型展開プラン|2億5,000万円(税別)",
       "60周年文脈を活用した全社大型コンシューマキャンペーン(6〜12か月)。初回稟議には大きく、B案実施後の拡張案として位置づけ")
add_table(s, Inches(0.7), Inches(1.75), Inches(6.0),
          ["見積項目", "金額"],
          [
...
add_table(s, Inches(0.7), Inches(1.75), Inches(11.9),
          ...
          [
              ...
              ["使用地域", "日本国内"],
              ["使用範囲", "イベント、BB、でんき、SoftBankショップ、Web、SNS"],
              ["使用形態", "キャンペーン告知、LP、店頭ツール、ノベルティ、イベント演出"],
              ["ヒーロー登場", "年間稼働枠を設定。追加稼働は別途"],
              ["ノベルティ", "作成数制限なし、数量に応じた許諾対価は別途"],
              ["サンプル納品/他IP併用/独占", "作成物ごとにサンプル提出/他IP併用可/独占は原則なし"],
              ["除外", "TVCM、交通広告、大型OOH、販売グッズ、アプリ改修、PayPayポイント原資"],
          ],
          col_widths=[1, 2.5], row_h=0.47, font_size=12.5)

# =============================================================================
# 26. スケジュール
# =============================================================================
s = add_slide()
header(s, "実施スケジュール案(FY26下期〜FY27本格展開)",
       "2026年度下期にA案パイロットで検証し、2027年4月からFY27年間契約(B案)へ移行します")
add_table(s, Inches(0.7), Inches(1.7), Inches(11.9),
          ["期間", "内容", "フェーズ"],
          [
              ["2026年7月〜8月", "初回提案・方向性合意", "準備"],
              ["2026年9月〜10月", "円谷プロ条件確認・概算見積精査", "準備"],
              ["2026年11月〜12月", "A案パイロット設計、商業施設候補選定", "検証"],
              ["2027年1月〜3月", "小規模検証イベント実施(A案)", "検証"],
              [("2027年4月", True, RED), ("FY27年間契約開始(B案)", True, RED), ("本展開", True, RED)],
              ["2027年5月〜6月", "春〜初夏イベント展開", "本展開"],
              ["2027年7月〜8月", "屋内商業施設中心の夏休み施策", "本展開"],
              ["2027年9月〜11月", "秋のファミリー送客施策", "本展開"],
              ["2027年12月〜2028年1月", "年末年始商業施設施策", "本展開"],
              ["2028年2月〜3月", "新生活・春商戦連動", "本展開"],
          ],
          col_widths=[1.1, 1.9, 0.55], row_h=0.42, font_size=12)
...
# =============================================================================
s = add_slide()
header(s, "最終提案:B案 FY27標準プラン", "ポケモン着ぐるみ不可後のリアルイベント機能を補完する本命施策")
add_table(s, Inches(0.7), Inches(1.8), Inches(11.9),
          ["項目", "内容"],
          [
              ["金額", ("1億2,000万円 税別", True, RED)],
...
textbox(s, Inches(0.85), Inches(6.05), Inches(11.8), Inches(0.9), [
    [("次アクション:", 14, RED, True),
     ("方向性ご合意のうえ、円谷プロ側への条件打診と概算見積の精査(2026年9月〜10月)、A案パイロットの設計(2026年11月〜)に着手します。", 14, INK, False)],
])
...
textbox(s, Inches(1.35), Inches(4.6), Inches(10.8), Inches(1.6), [
    ("ポケモン契約との整合/FY27以降の課題/商業施設送客/コンシューマ事業への貢献 — すべてが一本につながります。", 14.5, RGBColor(0xC3, 0xC9, 0xD2), False),
